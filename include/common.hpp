 ...
 * @returns The shortest left (negative) or right (positive) turn to get from h_0 to h_f
 */
 ...
 */
template <typename T>
Point<T> find_last_intersection(Point<T> centre, double radius, Segment<T> segment)
{
    ...
    if (std::min(x1, x2) - 0.0001 <= sol1x && sol1x <= std::max(x1, x2) + 0.0001)
    {
        sol1_valid = true;
    }
    if (std::min(x1, x2) - 0.0001 <= sol2x && sol2x <= std::max(x1, x2) + 0.0001)
    {
        sol2_valid = true;
    }
    if ((sol1_valid && sol2_valid) && (std::abs(sol1x - sol2x) < 0.001 && std::abs(sol1y - sol2y) < 0.001))
    {
        sol2_valid = false;
    }
 ...
 */
void find_intersections(Point<double> centre, double radius, Segment<double> segment, std::vector<Point<double>> &output);
 ...
 */
template <typename T>
T distance_between_points(Point<T> a, Point<T> b)
{
    double dx = (a.x - b.x);
    double dy = (a.y - b.y);
